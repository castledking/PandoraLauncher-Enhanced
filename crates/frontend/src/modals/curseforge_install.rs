                ...
                modal
                    ...
                    .child(Button::new("install").success().label(ts!("instance.content.install.label")).on_click(cx.listener(
                        move |this, _, window, cx| {
                            ...
                            let Some(url) = selected_file.download_url.clone() else {
                                window.push_notification((NotificationType::Error, "This file cannot be downloaded by third-party launchers"), cx);
                                return;
                            };
