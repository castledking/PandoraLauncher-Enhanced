...
fn open_version_picker_from_entity(
    ...
    let Some(instance) = data.instances.read(cx).entries.get(&install_for) else {
        open_error_dialog("Select version".into(), "Unable to find instance".into(), window, cx);
        return;
    };
    let instance = instance.read(cx);
    let minecraft_version = instance.configuration.minecraft_version.as_str();
        ...
        .filter(|version| {
            ...
            let matches_loader = match project_type {
                ModrinthProjectType::Mod | ModrinthProjectType::Modpack => {
                    instance_loader == Loader::Vanilla || loaders.contains(&instance_loader.as_modrinth_loader())
                },
                _ => true,
            };
    ...
    window.open_dialog(cx, move |dialog, _window, _cx| {
        ...
        for (index, version) in filtered.iter().enumerate() {
            ...
            content = content.child(Button::new(("version", index)).label(version_label).success().w_full().on_click(
                move |_, window, cx| {
                    window.close_all_dialogs(cx);
                    open_with_version(
