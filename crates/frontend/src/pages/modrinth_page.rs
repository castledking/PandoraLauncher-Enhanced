...
use bridge::instance::{
    AtomicContentUpdateStatus, ContentType, ContentUpdateStatus, InstanceContentID, InstanceContentSummary, InstanceID,
};
...
use crate::{
    component::{error_alert::ErrorAlert, page_path::PagePath},
    entity::{
        DataEntities,
                        ...
                        for (index, bundled_summary) in summaries.iter().enumerate() {
                            if let Some(bundled) = bundled_summary {
                                ...
                                if let Some(mod_id) = &bundled.id {
                                    let mod_id_key: Arc<str> = mod_id.to_lowercase().into();
                                    self.installed_mods_by_mod_id
                                        .entry(mod_id_key.clone())
                                        .or_insert(bundled_content.clone());
                                    self.installed_mods_by_filename_prefix
                                        .entry(mod_id_key)
                                        .or_insert(bundled_content.clone());
                                }
                ...
                for summary in resource_packs.iter() {
                    match &summary.content_source {
                        ...
                        ContentSource::ModrinthUnknown => {
                            if let Some(project) =
                                self.installed_resourcepacks_by_hash.get(&summary.content_summary.hash)
                            {
                                let project_key: Arc<str> = project.to_lowercase().into();
                                let installed = self.installed_resourcepacks_by_project.entry(project_key).or_default();
                                installed.push(InstalledContent {
                                ...
                                if let Some(filename) = path.file_name().and_then(|n| n.to_str()) {
                                    ...
                                    if !shader_name.is_empty() {
                                        ...
                                        let content = InstalledContent {
                                            ...
                                            status: Arc::new(AtomicContentUpdateStatus::new(
                                                ContentUpdateStatus::ManualInstall,
                                            )),
                                            mod_id: None,
                                        };
                                        self.installed_shaders_by_filename_prefix
    ...
    fn render_items(&mut self, visible_range: Range<usize>, _window: &mut Window, cx: &mut Context<Self>) -> Vec<Div> {
        ...
        let items = visible_range
            .map(|index| {
                ...
                // Fixed left slot so both rows align; same fixed width for primary and Open Page so they match
...
/// Small update-check nub shown to the left of Open Page when mod is installed.
#[derive(Clone, PartialEq, Eq)]
enum NubAction {
    CheckForUpdates,
    ErrorCheckingForUpdates,
...
impl PrimaryAction {
    pub fn text(&self) -> &'static str {
        match self {
            ...
            PrimaryAction::Installed => "Installed",
    ...
    pub fn icon(&self) -> Icon {
        match self {
            ...
            PrimaryAction::Installed => Icon::default().path("icons/check.svg"),
...
impl NubAction {
    fn icon(&self) -> Icon {
        match self {
            NubAction::CheckForUpdates => Icon::default().path("icons/refresh-ccw.svg"),
            NubAction::ErrorCheckingForUpdates => Icon::default().path("icons/triangle-alert.svg"),
            NubAction::UpToDate => Icon::default().path("icons/check.svg"),
            NubAction::Update(..) => Icon::empty().path("icons/download.svg"),
        }
    }
...
impl Render for ModrinthSearchPage {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let mut top_bar = h_flex()
            .w_full()
            .gap_3()
            .child(div().flex_1().min_w_0().child(Input::new(&self.search_state)));
