...
if [[ -n "$CARGO_PACKAGER_SIGN_PRIVATE_KEY" ]]; then
    ...
    \"downloads\": {
        \"x86_64\": {
            \"executable\": {
                \"download\": \"$GITHUB_REPO/releases/download/v$version/PandoraLauncher-Linux-x86_64-Portable\",
            ...
            \"appimage\": {
                \"download\": \"$GITHUB_REPO/releases/download/v$version/PandoraLauncher-Linux-x86_64.AppImage\",
    ...
    cp dist/update_linux.json dist/update_manifest_linux.json
