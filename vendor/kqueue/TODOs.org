...
*** TODO Add ~EventFilter~ to ~Event~ struct (good for removing items from coalescing vecdeque)
...
*** TODO async api
*** TODO bulk api
*** TODO `from_error` can falsely return errors, since not all errors fill `eventlist`
